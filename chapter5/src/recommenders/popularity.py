...
class PopularityRecommender(BaseRecommender):
    ...
    def recommend(self, dataset: Dataset, **kwargs) -> RecommendResult:
        ...
        movie_stats = self.dataset.train.groupby("movie_id").agg({"rating": [np.size, np.mean]})
        atleast_flg = movie_stats["rating"]["size"] >= minimum_num_rating
        movies_sorted_by_rating = movie_stats[atleast_flg].sort_values(by=("rating", "mean"),
    ...
    def _get_rating_pred(self):
        ...
        movie_rating_average = self.dataset.train.groupby("movie_id").agg({"rating": np.mean})
